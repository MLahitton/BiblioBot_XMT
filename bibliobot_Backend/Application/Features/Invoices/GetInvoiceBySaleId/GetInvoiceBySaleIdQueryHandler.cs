...
public sealed class GetInvoiceBySaleIdQueryHandler : IRequestHandler<GetInvoiceBySaleIdQuery, InvoiceDetailDto?>
{
    private readonly IApplicationDbContext _context;

    public GetInvoiceBySaleIdQueryHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task<InvoiceDetailDto?> Handle(
        GetInvoiceBySaleIdQuery request,
        CancellationToken cancellationToken)
    {
        var invoice = await _context.Invoices.AsNoTracking()
            ...
            .FirstOrDefaultAsync(i => i.SaleId == request.SaleId, cancellationToken);

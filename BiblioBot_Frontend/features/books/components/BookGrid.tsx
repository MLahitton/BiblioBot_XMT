...
function BookCover({
  ...
  return (
    <motion.div
      className="group relative w-[108px] shrink-0 cursor-pointer sm:w-[124px]"
    ...
    >
      {/* Projected shadow on the shelf below */}
      ...
      <div className="relative h-[160px] w-full overflow-hidden rounded-[7px] border border-white/50 bg-card shadow-[5px_12px_24px_rgba(53,30,28,0.24),inset_5px_0_0_rgba(0,0,0,0.18)] transition-all duration-500 group-hover:-translate-y-4 group-hover:rotate-[-1.5deg] group-hover:shadow-[8px_20px_38px_rgba(53,30,28,0.32),inset_5px_0_0_rgba(0,0,0,0.15)] sm:h-[184px]">
        ...
        {book.badge && (
          <span className="absolute bottom-2.5 left-1/2 max-w-[82%] -translate-x-1/2 rounded-full border border-white/30 bg-paper/92 px-2 py-1 text-center text-[0.52rem] font-black uppercase tracking-[0.08em] text-foreground shadow-sm whitespace-nowrap">
            {book.badge}
          </span>
        )}
      </div>
...
function BookInfo({ book }: { book: Book }) {
  return (
    <div className="flex w-[108px] shrink-0 flex-col items-center text-center sm:w-[124px]">
      ...
      {/* Stars */}
      <div className="mt-1 flex items-center justify-center gap-0.5 w-full">
        {[1, 2, 3, 4, 5].map((s) => (
          <svg
            key={s}
        ...
        <span className="ml-1 text-[0.58rem] font-bold text-muted">{book.rating.toFixed(1)}</span>
      </div>
  ...
  return (
    <div className="relative mx-0 mt-0">
      ...
      <div
        ...
        style={{
          height: "18px",
          background: "linear-gradient(180deg, #ecdbb8 0%, #d4bc8a 30%, #bfa06a 68%, #b08e56 100%)",
          boxShadow: "0 5px 14px rgba(53,30,28,0.20), inset 0 1px 0 rgba(255,255,255,0.50), inset 0 -1px 0 rgba(0,0,0,0.10)",
        }}
      >
        {/* Wood grain */}
        <span
          className="absolute inset-x-0 h-px opacity-[0.18] top-[5px]"
          style={{ background: "repeating-linear-gradient(90deg,transparent 0px,transparent 20px,rgba(53,30,28,0.8) 20px,rgba(53,30,28,0.8) 21px)" }}
        />
        <span
          className="absolute inset-x-0 h-px opacity-[0.10] top-[11px]"
          style={{ background: "repeating-linear-gradient(90deg,transparent 0px,transparent 34px,rgba(53,30,28,0.6) 34px,rgba(53,30,28,0.6) 35px)" }}
        />
      </div>
      {/* Underbelly shadow */}
      <div
        style={{
          height: "18px",
          background: "linear-gradient(180deg,rgba(60,30,10,0.20) 0%,rgba(53,30,28,0.06) 70%,transparent 100%)",
        }}
      />
      ...
      <div
        ...
        style={{
          height: "14px",
          background: "radial-gradient(ellipse at center, rgba(53,30,28,0.18) 0%, transparent 70%)",
        }}
      />
    </div>
  );
...
export function BookGrid({ books }: BookGridProps) {
  ...
  const relatedBooks = selectedBook
    ? books.filter((b) => b.id !== selectedBook.id && b.category === selectedBook.category).slice(0, 6)
    : [];
          ...
          >
            ...
            <div className="-mx-3 overflow-x-auto px-3 [scrollbar-color:rgba(53,30,28,0.18)_transparent] [scrollbar-width:thin]">
              <div className="min-w-max pr-4">
                ...
                {/* ── Row 2: Info cards (aligned below covers) ── */}
                <div className="flex gap-5 px-1 pt-4 pb-2 sm:gap-7">
                  {shelfBooks.map((book) => (

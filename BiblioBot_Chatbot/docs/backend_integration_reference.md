...
- Repositorio raíz: `BiblioBot_XMT`
- Proyectos:
...
- `Program.cs`:
  ...
  - soporte de seed por argumento `--seed`
    - La ejecucion del seed corresponde al equipo/backend, fuera del chatbot.
...
- El seed de desarrollo lo opera el backend por separado. FastAPI/chatbot no debe ejecutar comandos dotnet.
...
- - Nota backend: ADMIN debe tener permisos explícitos; FastAPI no debe asumir permisos por rol.

---

## 11) Comandos de operación y diagnóstico

- El backend se ejecuta por separado. El chatbot no debe ejecutar comandos dotnet ni modificar bibliobot_Backend.

---

## 12) Estado de pruebas

## Pendiente para fases posteriores: validar flujo E2E real con backend cuando se implemente cliente HTTP real.
...
- 4) El chatbot no debe modificar permisos del backend. Solo debe respetar los permisos recibidos desde ASP.NET Core.
